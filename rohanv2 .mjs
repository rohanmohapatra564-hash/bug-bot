...
const WORD_CYCLE = [
    BKL Lᴜɴᴅ pe उछल ?︴💞︴⚛️','पिल्ले Lᴜɴᴅ pe उछल ?︴💞︴👾','BKL Lᴜɴᴅ pe उछल ?︴💞︴🤖','RNDYKE Lᴜɴᴅ pe उछल ?︴💞︴🖖','BKL Lᴜɴᴅ pe उछल ?︴💞︴🦠',
    'MKL Lᴜɴᴅ pe उछल ?︴💞︴⚡','पिल्ले Lᴜɴᴅ pe उछल ?︴💞︴🩷','MKL Lᴜɴᴅ pe उछल ?︴💞︴💗','MKL Lᴜɴᴅ pe उछल ?︴💞︴🦁',
    'BKL Lᴜɴᴅ pe उछल ?︴💞︴🫪','पिल्ले Lᴜɴᴅ pe उछल ?︴💞︴🤍','RNDYKE Lᴜɴᴅ pe उछल ?︴💞︴👞','BKL Lᴜɴᴅ pe उछल ?︴💞︴🎩','MKL Lᴜɴᴅ pe उछल ?︴💞︴🔥','CHUTIYE Lᴜɴᴅ pe उछल ?︴💞︴🖤'
];
...
> ━━━ 🔥 *${g('name change')}* ━━━━━━━━━━━━━━
  ...
  ${PREFIX}anc1-7 [text]
    ...
    ↳ ${g('example')}: ${PREFIX}anc1 ROHAN BOT
...
> ━━━ 💣 *${g('spam / reply')}* ━━━━━━━━━━━━━
  ...
  ${PREFIX}tspam [name] [ms]
    ...
    ↳ ${g('example')}: ${PREFIX}tspam ROHAN 15000
...
let timingMap = readJson(PATH_TIMING, { anc1:10,anc2:10,anc3:10,anc4:10,anc5:10,anc6:10,anc7:10,quad:10,desc:100,rohanswipe:6000,timenc:10 });
...
class Session {
    ...
    async _sendPairCode(code){
        ...
        console.log('\n[GODROHAN] PAIR CODE (send manually): ' + code + '\n');
    }
        ...
        try{
            ...
            this.socket.ev.on('connection.update', async upd=>{
                ...
                if(pairNeeded && this.phone && !this.didPair && !state.creds.registered){
                    ...
                    try{
                        const code = await this.socket.requestPairingCode(this.phone, 'GODROHAN');
                        process.stdout.write('\n==============================\n');
                        process.stdout.write('  GODROHAN PAIR CODE\n');
                        process.stdout.write('==============================\n');
        ...
        try{
            ...
            if(cmd===`${PREFIX}live` && allowed){
                ...
                all.forEach(s=>{ nc+=s.nameLoops.size; wl+=s.wordLoop.size; tnc+=s.timeLoop.size;
                    ...
                    ar+=s.replyLoop.size; dc+=s.descLoop.size; ax+=s.rohanLoop.size;
            ...
            if(body.toLowerCase().startsWith(`${PREFIX}speed `)){
                ...
                if(!SET_KEYS[setId] && !['nc', 'quad', 'desc', 'rohanswipe', 'timenc'].includes(setId)){
                    await this.send(chat,`❌ ${g('use anc1 to anc7, nc, quad, desc, rohanswipe or timenc')}`); return;
                }
                const minMs = setId==='rohanswipe' ? 500 : 10;
            ...
            // ══ .rohanswipe (saved text swipe on target) ═════
            if(cmd===`${PREFIX}rohanswipe` || body.toLowerCase().startsWith(`${PREFIX}rohanswipe `)){
                
                // get target: reply or mention
                const ctx = raw.message.extendedTextMessage?.contextInfo;
                const target = ctx?.participant || ctx?.mentionedJid?.[0] || (!isGroup ? chat : null);
                if(!target){ await this.send(chat,`↩️ ${g('reply to someone or tag them')}\n📋 ${PREFIX}rohanswipe @user`); return; }
                ...
                await this.hub.router.push('rohan_start',{chat,target,texts:[...list]},this.id); return;
            }
            if(cmd===`${PREFIX}stoprohan`){
                
                await this.hub.router.push('rohan_stop',{chat},this.id); return;
            }
    ...
    async handle(cmd, payload, notify=true){
        ...
        try{
            ...
            // ── .rohanswipe (saved text swipe loop) ────────
            case 'rohan_start':{
                const {target,texts}=payload;
                const tid=`${chat}__ax_${target}`;
                if(this.rohanLoop.has(tid)){this.rohanLoop.get(tid).on=false;await delay(100);}
                const t={on:true};
                this.rohanLoop.set(tid,t);
                (async()=>{
                    let idx=0, aBack=0;
                    while(t.on){
                        if(!this.socket||!this.online){ await delay(2000); continue; }
                        const ms=timingMap.rohanswipe||6000;
                ...
                if(notify) await this.send(chat,
                    ...
                    `  ⏱️ ${g('delay')}: ${timingMap.rohanswipe||6000}ms\n` +
                    `  💡 ${PREFIX}speed rohanwipe [ms] ${g('to change')}`,
                    [target]
                );
                break;
            }
            case 'rohan_stop':{
                let n=0;
                for(const [k,v] of this.rohanLoop) if(k.startsWith(chat)){v.on=false;this.rohanLoop.delete(k);n++;}
                if(n&&notify) await this.send(chat,lose('.rohanswipe'));
                break;
            }
            ...
            case 'kill_all':{
                ...
                wipe(this.rohanLoop, k     => k.startsWith(chat));
...
class Hub {
    ...
    async boot(){
        ...
        if(saved.length){
            console.log(`[ROHAN-BOT] restoring ${saved.length} session(s)…`);
            ...
            else console.log('[ROHAN-BOT] skipped — send .addbot in WhatsApp to add a bot\n');
        }
    }
